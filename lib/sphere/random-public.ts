...
// (TABLESAMPLE strategy, server-side — see migration 0003). Callable with the
// anon client. Returns null when the public pool is empty (cold start, D4) or on
// error. M4's /discover wires this to a "next" button + camera warp.
export async function getRandomPublicSphere(
  supabase: SupabaseClient,
): Promise<RandomPublicSphere | null> {
  const { data, error } = await supabase
    .rpc("random_public_sphere")
    .maybeSingle();
